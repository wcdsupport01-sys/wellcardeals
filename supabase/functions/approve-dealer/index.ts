...
//      dealer_id) so the dealer can log in.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (createErr) {
      ...
      // Reset their password to the new temp one so the emailed credentials
      // actually work.
    ...
    return jsonResponse({ ok: true, ...updatedApp, emailResult });
  } catch (err) {
